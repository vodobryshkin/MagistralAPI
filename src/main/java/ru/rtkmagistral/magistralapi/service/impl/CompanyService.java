...
import ru.rtkmagistral.magistralapi.exception.CompanyCreditsException;
...
public class CompanyService implements ICompanyService {
    ...
    public boolean verifyData(CreateCompanyRequest req) {
        ...
        if (companyRequestFormClient == null) {
            throw new CompanyCreditsException("COMPANY_WITH_THIS_INN_NOT_EXISTS_IN_DADATA");
        }

        if (!titleReq.equals(companyRequestFormClient.getTitle())) {
            throw new CompanyCreditsException("TITLE_NOT_MATCHES_WITH_DADATA");
        }

        if (!innReq.equals(companyRequestFormClient.getInn())) {
            throw new CompanyCreditsException("INN_NOT_MATCHES_WITH_DADATA");
        }

        if (!kppReq.equals(companyRequestFormClient.getKpp())) {
            throw new CompanyCreditsException("KPP_NOT_MATCHES_WITH_DADATA");
        }

        if (!okvedReq.equals(companyRequestFormClient.getOkved())) {
            throw new CompanyCreditsException("OKVED_NOT_MATCHES_WITH_DADATA");
        }
    ...
    public void createCompany(CreateCompanyRequest createCompanyRequest, User user) {
        if (verifyData(createCompanyRequest)) {
            if (companyRepository.existsCompanyByInn(createCompanyRequest.getInn())) {
                throw new CompanyCreditsException("COMPANY_ALREADY_EXISTS_IN_DATABASE");
            }
            ...
            throw new CompanyCreditsException("PROBLEMS_WHILE_ADDING_A_COMPANY");
        }
    }
}
