...
@WebMvcTest(SuitcaseController.class)
@Import({AppExceptionHandler.class, WebTestSupport.class})
class SuitcaseControllerIT {

    @Autowired
    MockMvc mvc;

    @MockitoBean
    ISuitcaseService suitcaseService;
    ...
    private static final String VALID_JSON = """
        {
            ...
            "comment": "Отвезти в ООО «Ромашка», 3-й этаж.",
    ...
    @Test
    @DisplayName("POST /suitcases без Idempotency-Key — 400")
    ...
    @Test
    @DisplayName("POST /suitcases с пустым адресом отправителя — 422")
    void blankShippingAddress_returns422() throws Exception {
    ...
    @Test
    @DisplayName("POST /suitcases с одновременно deliver_as_soon_as_possible=true и wishing_delivery_time — 422")
    ...
    @Test
    @DisplayName("POST /suitcases с несогласием с договором — 422")
    ...
    @Test
    @DisplayName("POST /suitcases когда заявка ещё создаётся — 409 ORDER_IS_STILL_BEING_CREATED")
    void suitcaseStillBeingCreated_returns409() throws Exception {
        when(suitcaseService.createIdempotentSuitcase(any(), any(), any(), any(), any()))
